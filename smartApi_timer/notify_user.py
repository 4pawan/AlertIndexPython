...
class NotifyUser:
    ...
    def send_message(data: Result):
          ...
          tu.send_telegram_message(f"{data.Signal}{strgth}  change {data.Change}  close {data.Close} oi  {data.OI}   vol {data.Vol}",config.telegram_token,config.telegram_chat_id)
